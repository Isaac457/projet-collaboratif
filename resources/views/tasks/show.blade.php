...
<div class="container mx-auto p-6">
    ...
    <div class="mt-6 bg-white shadow-md rounded-lg p-4">
        ...
        <p class="mt-2 text-gray-600"><strong>Statut :</strong> <span class="px-2 py-1 text-sm rounded-full bg-[#76CDCD] text-white">{{ $task->status }}</span></p>
        <div class="mt-4">
            <a href="{{ route('tasks.edit', [$project, $task]) }}" class="bg-[#F27438] text-white px-4 py-2 rounded-lg">Modifier</a>
            <form action="{{ route('tasks.destroy', [$project, $task]) }}" method="POST" class="inline-block ml-2">
                @csrf
                @method('DELETE')
